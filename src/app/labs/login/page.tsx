...
import { readLabsSession } from "@/lib/auth.server";

export const dynamic = "force-dynamic";

type Search = { next?: string; err?: string; uid?: string };

export default async function LabsLoginPage({ searchParams }: { searchParams: Promise<Search> }) {
  const { next = "/labs", err, uid } = (await searchParams) || {};
    ...
    <main className="flex min-h-[80vh] items-center justify-center bg-[#f7f2ea] px-4 py-16">
      <div className="w-full max-w-md space-y-6 rounded-[32px] bg-white p-8 shadow-2xl ring-1 ring-slate-100">
        <div className="space-y-1 text-center">
          ...
          <h1 className="text-2xl font-semibold text-slate-900">Sign in to manage lab orders</h1>
          <p className="text-sm text-slate-500">Use the whitelisted labs account to continue.</p>
        </div>
        <form method="POST" action="/api/labs/login" className="space-y-4">
        ...
        <p className="text-center text-xs text-slate-500">
          Need access? Contact CalDoc support to add your labs account.
        </p>
      </div>
    </main>
  );
