...
export default async function LabEnrollmentsPage({
  searchParams,
}: {
  ...
  const [counts, enrollments] = await Promise.all([
    prisma.labEnrollment.groupBy({
      by: ["status"],
      _count: { status: true },
    }),
    prisma.labEnrollment.findMany({
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        {tabs.map((tab) => (
          <Link
            key={tab.value}
            href={`/admin/enrollments/labs?status=${tab.value}`}
      ...
      <div className="overflow-hidden rounded-3xl border border-white/70 bg-white/90 shadow-[0_4px_24px_-4px_rgba(88,110,132,0.15)]">
        {enrollments.length === 0 ? (
          <div className="py-16 text-center text-slate-400">
            No {filterStatus !== "ALL" ? filterStatus.toLowerCase() : ""} lab enrollments found.
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 text-left text-xs font-semibold uppercase tracking-wide text-slate-400">
                  {["Lab", "Contact", "Location", "Certifications", "Submitted", "Status", ""].map(
                    (h) => (
                ...
                {enrollments.map((e) => (
                  <tr key={e.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/50">
                    <td className="px-4 py-3">
                      <div className="font-semibold text-slate-900">{e.labName}</div>
                    ...
                    <td className="px-4 py-3">
                      <Link
                        href={`/admin/enrollments/labs/${e.id}`}
