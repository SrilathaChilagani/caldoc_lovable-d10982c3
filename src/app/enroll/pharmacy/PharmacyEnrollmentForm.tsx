...
export default function PharmacyEnrollmentForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/enroll/pharmacy", { method: "POST", body: data });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className={cardCls}>
        <h2 className="mb-4 text-sm font-semibold text-slate-900">Pharmacy Details</h2>
        <div className="grid gap-3 sm:grid-cols-2">
          <label className={`${labelCls} sm:col-span-2`}>
            Pharmacy name *
            <input name="pharmacyName" required className={inputCls} placeholder="e.g. MedPlus Pharmacy" />
          </label>
          <label className={labelCls}>
            Contact person name *
            <input name="contactName" required className={inputCls} placeholder="e.g. Ravi Kumar" />
          </label>
          <label className={labelCls}>
            Email address *
            <input name="email" type="email" required className={inputCls} placeholder="pharmacy@email.com" />
          </label>
          <label className={labelCls}>
            Phone / WhatsApp *
          ...
          <label className={labelCls}>
            Drug License Number *
            <input name="drugLicenseNumber" required className={inputCls} placeholder="e.g. AP/DL/2023/12345" />
          </label>
          <label className={labelCls}>
            GST Number (optional)
            <input name="gstNumber" className={inputCls} placeholder="e.g. 37AABCT1234Z1Z5" />
          </label>
        </div>
      </div>
      ...
      {/* Service Areas */}
      <div className={cardCls}>
        <h2 className="mb-4 text-sm font-semibold text-slate-900">Service Areas &amp; Notes</h2>
        <div className="space-y-3">
          <label className={labelCls}>
            ...
            <textarea
              ...
              rows={2}
              className={inputCls}
              placeholder="e.g. Hyderabad, Secunderabad, Kukatpally"
            />
            <span className="mt-0.5 block text-[10px] text-slate-400">Enter the areas or pin codes you can deliver to.</span>
          </label>
          <label className={labelCls}>
