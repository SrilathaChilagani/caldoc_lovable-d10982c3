...
import { clearRxCart } from "./rxCart";
...
export default function RxOrderCheckoutClient({ orderId }: Props) {
  ...
  useEffect(() => {
    async function start() {
      try {
        ...
        setError(null);
        const res = await fetch("/api/rx-orders/create-order", {
          method: "POST",
        ...
        const rzp = new window.Razorpay({
          ...
          name: "CalDoc Rx Delivery",
          theme: { color: "#0f172a" },
          prefill: data.prefill,
          handler: async (response: RazorpayResponse) => {
            try {
              setStatus("confirming");
              const confirmRes = await fetch("/api/rx-orders/confirm", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
              ...
              setStatus("success");
              clearRxCart();
              window.location.href = `/services/rx-delivery/success?order=${orderId}`;
            } catch (err) {
