...
import { fetchLabOrders, labTestNames, formatAmount, type LabOrder } from '../lib/api';
...
const STATUS_COLORS: Record<string, { bg: string; text: string }> = {
  ...
  SAMPLE_COLLECTED: { bg: '#F0FDFA', text: '#0891B2' },
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (e instanceof Error && e.message === 'session_expired') {
        ...
        setError('Could not load lab orders. Pull down to retry.');
      }
    } finally {
      setLoading(false);
      setRefreshing(false);
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.center}>
            ...
            <Text style={styles.emptySubtitle}>Your lab tests will appear here.</Text>
          </View>
        }
        renderItem={({ item }) => {
          const tests = labTestNames(item.tests);
          const { bg, text: textColor } = statusStyle(item.status);
          return (
            <View style={styles.card}>
              <View style={styles.cardTop}>
                <View style={styles.iconCircle}>
                  <Ionicons name="flask" size={20} color="#0F62FE" />
                </View>
                ...
                <View style={[styles.badge, { backgroundColor: bg }]}>
                  <Text style={[styles.badgeText, { color: textColor }]}>{item.status.replace(/_/g, ' ')}</Text>
                </View>
              </View>
                ...
                <View style={styles.testsList}>
                  {tests.map((t, i) => (
                    <View key={i} style={styles.testRow}>
                      <Ionicons name="checkmark-circle-outline" size={14} color="#0F62FE" />
