...
export default function EnrollmentActions({
  ...
  async function handleApprove() {
    if (!confirm("Approve this provider? This will create their portal account and send them login credentials via WhatsApp and email.")) return;
    setApproving(true);
    setError(null);
    try {
      ...
      setSuccess(`Provider approved! Temporary password: ${data.tempPassword}\nSent via WhatsApp to provider's mobile.`);
  ...
  return (
    <div className="rounded-2xl border border-white/70 bg-white/90 p-6 shadow-sm space-y-4">
      ...
      {showRejectForm && (
        <div className="space-y-3 pt-2 border-t border-slate-100">
          <label className="block text-xs font-medium text-slate-600">
            Reason for rejection (will be recorded internally)
            <textarea
              rows={3}
              value={reason}
              onChange={(e) => setReason(e.target.value)}
              placeholder="e.g. Registration certificate could not be verified. Please reapply with a valid document."
