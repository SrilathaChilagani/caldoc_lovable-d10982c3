...
export default function RxStatusAction({
  orderId,
  currentStatus,
}: {
  orderId: string;
  ...
  return (
    <>
      <button
        ...
        className="rounded-full border border-[#2f6ea5]/30 px-3 py-1 text-xs font-semibold text-[#2f6ea5] hover:bg-[#e7edf3]"
      >
        Update status
      </button>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4">
          <div className="w-full max-w-sm rounded-3xl bg-white p-6 shadow-2xl">
            <h2 className="font-serif text-lg font-semibold text-slate-900">Update Rx order status</h2>
