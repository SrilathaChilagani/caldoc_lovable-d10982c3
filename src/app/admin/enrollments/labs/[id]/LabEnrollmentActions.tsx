...
export default function LabEnrollmentActions({
  ...
  async function handleApprove() {
    if (!confirm("Approve this lab? This will create a lab partner account and notify them via WhatsApp.")) return;
    setApproving(true);
    setError(null);
    try {
      const res = await fetch(`/api/admin/enrollments/labs/${enrollmentId}/approve`, {
        method: "POST",
  ...
  async function handleReject() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/admin/enrollments/labs/${enrollmentId}/reject`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        ...
        <div className="space-y-3 border-t border-slate-100 pt-2">
          <label className="block text-xs font-medium text-slate-600">
            ...
            <textarea
              ...
              placeholder="e.g. NABL certificate could not be verified. Please reapply with a valid document."
