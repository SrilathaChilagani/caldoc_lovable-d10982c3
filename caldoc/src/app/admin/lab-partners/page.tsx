...
export default async function LabPartnersPage({
  ...
  if (!sess) redirect("/admin/login?next=/admin/lab-partners");
  ...
  const [partners, totalActive, totalInactive] = await Promise.all([
    ...
    prisma.labPartner.count({ where: { isActive: true } }),
    prisma.labPartner.count({ where: { isActive: false } }),
  ]);
  ...
  return (
    <>
      <div>
        ...
        <h1 className="font-serif text-3xl font-semibold text-slate-900">Lab Partners</h1>
        <p className="mt-1 text-sm text-slate-500">
          Manage partner diagnostic labs that fulfil at-home sample collection and testing.
        </p>
      </div>

      {sp.created && (
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-800">
          Lab partner onboarded successfully.
        </div>
      )}
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <div className="flex flex-col justify-center rounded-2xl border border-white/70 bg-white/90 p-4 shadow-[0_4px_24px_-4px_rgba(88,110,132,0.15)]">
          <Link
            href="/admin/lab-partners/onboard"
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-white/70 bg-white/90 p-4 shadow-[0_4px_24px_-4px_rgba(88,110,132,0.15)]">
        <div className="flex gap-1.5">
          {[
            ...
            <Link
              key={opt.label}
              href={`/admin/lab-partners?active=${opt.value}${query ? `&q=${encodeURIComponent(query)}` : ""}`}
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {partners.length === 0 ? (
          <p className="col-span-full py-10 text-center text-sm text-slate-400">No lab partners found.</p>
        ) : (
          partners.map((p) => (
            <div key={p.id} className="rounded-3xl border border-white/70 bg-white/90 p-5 shadow-[0_4px_24px_-4px_rgba(88,110,132,0.15)]">
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {p.nablCertified && (
                      <span className="shrink-0 rounded-full bg-blue-100 px-2 py-0.5 text-[10px] font-bold text-blue-700">NABL</span>
                    )}
                  </div>
              ...
              <div className="mt-3 space-y-1 text-xs text-slate-600">
                ...
                <p>
                  <span className="font-medium text-slate-700">Home collection:</span>{" "}
                  <span className={p.homeCollection ? "text-emerald-600" : "text-slate-400"}>
                    {p.homeCollection ? "Available" : "Not available"}
                  </span>
                </p>
                {p.nablCertNumber && (
                  <p><span className="font-medium text-slate-700">NABL cert:</span> {p.nablCertNumber}</p>
                )}
                {p.testCategories.length > 0 && (
                  <p><span className="font-medium text-slate-700">Tests:</span> {p.testCategories.join(", ")}</p>
                )}
              </div>
